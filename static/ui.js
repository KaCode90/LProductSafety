...
export function badge(status){const good=['Đang sử dụng','Đạt yêu cầu','Compliant','Pass','PASS','Valid','Closed','Completed','Approved'];const bad=['NG','FAIL','Expired','Overdue','Rejected','Critical'];const warn=['Tạm ngưng','Thiếu hồ sơ','Cần cập nhật','Expiring Soon','Due Soon','Warning','Pending Verification'];return `<span class="badge ${good.includes(status)?'good':bad.includes(status)?'bad':warn.includes(status)?'warn':''}">${esc(status||'Pending')}</span>`;}
