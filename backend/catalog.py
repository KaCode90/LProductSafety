...
for key, title in [('cts-2','CTS_2 – IQC / OQC'),('cts-3','CTS_3 – VOC / SVHC')]:
    module(key,title,'materials',REQUIREMENT,'Quản lý requirement, review, DRI và evidence.')
module('cts-1', 'CTS_1 – Chất hạn chế', 'materials', RSS_FIELDS, 'Danh mục tiêu chuẩn chất hạn chế của Apple (Apple Regulated Substances Specification).', common=False)
...
module('materials','Danh mục NVL','materials',MATERIAL,'Mở một vật liệu để xem toàn bộ FMD, TRM, XRF, tài liệu và lịch sử.')
module('suppliers','Nhà cung cấp','materials',fields([('supplier','Nhà cung cấp','text',None,True),('manufacturer','Nhà sản xuất'),('contact','Người liên hệ'),('email','Email','email'),('phone','Điện thoại'),('address','Địa chỉ','textarea')]))
...
module('bom','BOM / Sản phẩm','materials',fields([('customer','Customer'),('config','Config'),('parent_code','Mã thành phẩm'),('project','Model'),('category','Category'),('level','Level','number'),('material_code','Part Code','text',None,True),('material_name','Part Name'),('supplier','Supplier'),('norm','NORM')]))
module('test-plan','Kế hoạch kiểm nghiệm','materials',fields([('project','Model'),('material_code','Part Code'),('material_name','Part Name','text',None,True),('test_type','Loại test'),('test','IQC / OQC'),('category','Category'),('sample_size','Sample size'),('availability','Availability'),('cp','CP'),('oct','Oct 2025'),('nov','Nov 2025'),('dec','Dec 2025')]))
module('oqc-reports','Báo cáo thành phẩm / OQC','materials',REPORT)
module('xrf-plan','XRF – Kế hoạch NPI / MP','materials',fields([('project','Model'),('config','Config'),('material_code','Part Code','text',None,True),('old_code','Old Part Code'),('material_name','Part Name'),('supplier','Supplier'),('test','IQC / OQC'),('category','Category'),('thickness','Độ dày / pcs (µm)'),('sample_size','Số lớp / số pcs'),('npi_frequency','Tần suất NPI'),('mp_frequency','Tần suất MP')]) + [field('plan_'+m,m,'select',['Planned','Not planned','NFV1 Control']) for m in ['2025-10','2025-11','2025-12']+['2026-'+str(i).zfill(2) for i in range(1,13)]])
for key,title in [('xrf-iqc','XRF – Kết quả IQC'),('xrf-oqc','XRF – Kết quả OQC'),('change-control','XRF – Change Control')]:
    module(key,title,'materials',XRF,'Đánh giá sàng lọc theo Control Limit của sheet Standard; không thay thế phê duyệt compliance.')
